...
namespace ZArch {
    public sealed partial class ArchitectureScope {
        private sealed class TrackedEventUnregister : IUnregister {
            ...
            public TrackedEventUnregister(ArchitectureScope owner, IUnregister unregister, Delegate handler) {
                m_Owner = owner;
                m_Unregister = unregister;
                m_Handler = handler;
            }

            public bool Matches(Delegate handler) => Equals(m_Handler, handler);
        ...
        public IUnregister Subscribe<T>(Action<T> onEvent) {
            ...
            var tracked = new TrackedEventUnregister(this, m_Events.Subscribe(onEvent), onEvent);
